...
async def send_yomiyasu_embed(message: Message, db: Collection, yomiyasu_db: Collection):
    if "manga.manabe.es/app/series" in message.content and len(message.embeds) > 0:
        await message.edit(suppress=True)
        url_pattern = r'https?://\S+|www\.\S+'
        urls = re.findall(url_pattern, message.content)
        embeds = []
        for url in urls:
            if "manga.manabe.es/app/series" in message.content:
                ...
                serie_embed.set_author(
                    name="Yomiyasu", url="https://manga.manabe.es/")
